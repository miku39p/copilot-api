/* eslint-disable max-lines, max-lines-per-function, complexity, max-depth, eqeqeq, no-nested-ternary, unicorn/prefer-single-call, @typescript-eslint/no-unnecessary-condition, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-explicit-any, @typescript-eslint/no-this-alias, @typescript-eslint/no-dynamic-delete, @typescript-eslint/no-non-null-assertion */
import {
  APICallError,
...
export class OpenAIResponsesLanguageModel implements LanguageModelV2 {
  ...
  private async getArgs({
    ...
    const topLogprobs =
      typeof openaiOptions?.logprobs === "number" ? openaiOptions?.logprobs
      : openaiOptions?.logprobs === true ? TOP_LOGPROBS_MAX
      : undefined
          ...
          transform(chunk, controller) {
            ...
            if (isResponseOutputItemAddedChunk(value)) {
              switch (value.item.type) {
                ...
                default: {
                  if (isResponseOutputItemAddedReasoningChunk(value)) {
              ...
              switch (value.item.type) {
                ...
                default: {
                  if (isResponseOutputItemDoneReasoningChunk(value)) {
                ...
                controller.enqueue({
                  ...
                  providerMetadata: {
                    openai: {
                      ...
                      reasoningEncryptedContent:
                        activeItem.encryptedContent ?? null,
                    },
                  },
                })
              }
